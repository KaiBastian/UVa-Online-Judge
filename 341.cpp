...
private:
    std::vector<StoredType>
        heap_; // indexed starting w/1 bc of getParent w/o branching
...
template<typename ValueType>
typename PriorityQueue<ValueType>::StoredType PriorityQueue<ValueType>::top()
{
    if (heap_.empty())
...
template<typename ValueType>
typename PriorityQueue<ValueType>::PrioType PriorityQueue<ValueType>::getPrio(
    ValueType elem)
{
...
void PriorityQueue<ValueType>::bubbleUp(HeapLocationType start)
{
    for (HeapLocationType current = start, parent = getParent(start);
         current != parent; current = parent, parent = getParent(current))
    {
...
{
    for (HeapLocationType current = start,
                          smallerChild = getSmallerChild(start);
...
public:
    ...
    WeightType singleSourceShortesPath(
        NodeType from,
        NodeType to,
...
Graph::WeightType Graph::singleSourceShortesPath(
    Graph::NodeType from,
    Graph::NodeType to,
    std::deque<Graph::NodeType>& route)
{
    ...
    while (!unvisitedNodes.empty())
    {
        ...
        for (const auto edge : adjacencyList)
        {
            ...
            {
                unvisitedNodes.updateOrPush(
                    std::make_pair(targetNode, newWeight));
...
void Graph::calculateRoute(
    ...
    std::deque<NodeType>& route)
{
    for (NodeType currentNode = to; currentNode != from;
...
int main()
{
    ...
    while (true)
    {
        ...
        const unsigned weight =
            graph.singleSourceShortesPath(from - 1, to - 1, route);

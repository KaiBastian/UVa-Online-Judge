...
{
public:
    NTimesNMatrix(size_t n, unsigned defaultValue)
        : values_(n * n, defaultValue), n_(n)
    {
    }
    unsigned& get(unsigned row, unsigned column)
    {
        return values_[n_ * row + column];
    }
...
NTimesNMatrix Graph::floydWarshall() const
{
    ...
    NTimesNMatrix distances(
        numberOfNodes, std::numeric_limits<unsigned>::max() / 4);
    ...
    for (size_t includedNode = 0; includedNode < numberOfNodes; ++includedNode)
    {
        for (size_t from = 0; from < numberOfNodes; ++from)
        {
            ...
            {
                const unsigned detourWeight = distances.get(from, includedNode)
                                              + distances.get(includedNode, to);
                unsigned& fromToWeight = distances.get(from, to);
...
void solveTestcase(const Graph& graph, unsigned testcaseNumber)
{
    ...
    std::cout << "Case " << testcaseNumber
              << ": average length between pages = " << avgWeight
              << " clicks\n";
}

unsigned lookupIntern(
    std::unordered_map<unsigned, unsigned>& extern2intern,
    unsigned externValue,
    unsigned& nextIntern)
{
...
bool parseAndSolveTestcase(unsigned testcaseNumber)
{
    ...
    while (std::cin >> from >> to && ((from != 0) & (to != 0)))
    {
        ...
        const unsigned fromIntern =
            lookupIntern(extern2intern, from, nextIntern);

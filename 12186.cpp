...
{
    EmployeeRecord() : directReports(), costToActivate(1), properlyEvaluated(false) {}
	...
	EmployeeRecord& managerRecord)
{
	...
	std::sort(directReportsActivationCost.begin(), directReportsActivationCost.end());
	
	return directReportsActivationCost;	
}

unsigned calculateCost(const std::vector<unsigned>& costs, unsigned percentPoints)
{
	unsigned cost = 0;
	unsigned numberOfNeededEmp = 
...
{
	const auto listOfCheapEmps = 
		getCheapEmployees(employees, managerRecord);
...
void calculateActivationCost(std::vector<EmployeeRecord>& employees, unsigned percentPoints)
{
    ...
    while (!employeesToExplore.empty())
    {
		...
		{
			// We have a manager.
			// If all his direct reports were processed already, we can evaluate him.
		...
		else
		{
			...
			// We don't clear the manager from the stack, cause he is still unevaluated.
		}
    }
} 
...
int main()
{
	...
	while (true)
	{
        ...
        for (unsigned empNumber = 1; empNumber <= numberOfEmployees; ++empNumber)
        {
            unsigned hisManager;
            std::cin >> hisManager;

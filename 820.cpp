...
public:
    Matrix(size_t numRows, size_t numColumns)
        : numColumns_(numColumns), data_(numRows * numColumns, 0u)
    {
    }
    ...
    size_t getNumRows() const { return data_.size() / numColumns_; }

    size_t getNumColumns() const { return numColumns_; }
...
{
public:
    explicit Graph(size_t numNodes) : capacityMatrix_(numNodes, numNodes) {}
    ...
    int findMaxFlow(size_t s, size_t t)
    {
        ...
        while (true)
        {
            ...
            for (size_t curr = t; curr != stPathPred[curr];
                 curr = stPathPred[curr])
            {
...
private:
    size_t getNumNodes() const { return capacityMatrix_.getNumColumns(); }
